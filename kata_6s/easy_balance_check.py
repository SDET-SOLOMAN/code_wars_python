...
def balance(book: str) -> str:
    book = ''.join(ch for ch in book if ch.isalnum() or ch in '. \n')
    spl = [' '.join(line.split()) for line in book.strip().split('\n') if line.strip()]
    
    bal = float(spl[0])
    arr = [f'Original Balance: {bal:.2f}']
    ...
    for el in spl[1:]:
        ...
        cnt += 1
        bal -= price
        arr.append(f'{" ".join(parts[:-1])} {price:.2f} Balance {bal:.2f}')
    
    arr.append(f'Total expense  {total:.2f}\r\nAverage expense  {(total/cnt if cnt else 0):.2f}')
    return '\r\n'.join(arr)
