...
# The Morse code is case-insensitive, traditionally capital letters are used. When
# the message is written in Morse code, a single space is used to separate the character
...
# variable warning for morse_codes because it's no longer used and kept only for old solutions.
...
# and exceptions. In C#, tests will fail if the solution code throws an exception,
...
def decode_morse(morse_code):
    mc = morse_code.strip().split(" ")
    print(mc)

    r_s = ""
    sp = False

    for char in range(len(mc)):

        try:

            if MORSE_CODE.get(mc[char]) and mc[char - 1] == "":
                r_s += " " + (MORSE_CODE[mc[char]])

            elif MORSE_CODE.get(mc[char]):
                r_s += (MORSE_CODE[mc[char]])
