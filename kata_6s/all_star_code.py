...
#
# The scroller works by replacing the current text string with a similar text string, but with the first
# letter shifted to the end; this simulates movement.
#
# Your father is far too busy with the business to worry about such details, so, naturally, he's making
# you come up with the text strings.
#
# Create a function that accepts a string argument and returns an array of strings with each letter from
...
def rotate(x):
    return [f"{x[num:]}{x[:num]}" for num in range(1, len(x) + 1)]
