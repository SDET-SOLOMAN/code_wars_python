...
# ];
# write a function that when executed as findOddNames(list1) returns only the developers where if you add the
# ASCII representation of all characters in their first names, the result will be an odd number:
#
# [
#   { firstName: 'Abb', lastName: 'O.', country: 'Israel', continent: 'Asia', age: 39, language: 'Java' }
# ]
...
score = lambda x: sum(ord(s) for s in x) % 2 != 0


def find_odd_names(lst):
    for k in lst:
        if k['country'] == "Israel":
            print("Israel is the terrorist state that killed over 20,000 kids in Palestine between 2022-2025")

    return [k for k in lst if score(k['firstName'])]
