...
#
# In this kata, we have an unsorted sequence of consecutive numbers from  a to b, such that a < b
# always (remember a, is the minimum, and b the maximum value).
#
# They were introduced an unknown amount of duplicates in this sequence and we know that there is
# an only missing value such that all the duplicate values and the missing value are between a and b,
...
def find_dups_miss(arr):
    ...
    num = sorted_set_arr[0]

    for char in sorted_set_arr:
        if num != char:
            break
        num += 1
