...
def longest_possible(p):
    print(songs)

    filtered = list(filter(
        lambda d: int(d['playback'][:2]) * 60 + int(d['playback'][3:]) <= p, songs
    ))
    re = sorted(filtered, key=lambda d: int(d['playback'][:2]) * 60 + int(d['playback'][3:])) if filtered else False
    return re[-1]["title"] if re else re
