...
#
# Each year, Society for Exciting Computer Science Research (SECSR) organizes several conferences
# around the world. Lucy always picks one conference from that list that is hosted in a city she hasn't
# been to before, and if that leaves her with more than one option, she picks the conference that she thinks
...
def conference_picker(cities_visited, cities_offered):
    v = list(filter(lambda x: x not in cities_visited, cities_offered))
    if v:
        return f"{v[0]}"
