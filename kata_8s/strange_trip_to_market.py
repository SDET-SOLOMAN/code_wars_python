...
# As you wonder what kind of algorithm you could use to shift octaves by 8 pitches or something silly l
# ike that, it dawns on you that you have been watching the musician for some 10 odd minutes. You ask,
...
# "tree fiddy". Since you are tired of being grifted by this monster, the time has come to code
# a solution for finding The Loch Ness Monster. Note that the phrase can also be written as "3.50" or "three fifty".
# Your function should return true if you're talking with The Loch Ness Moster, false otherwise.


def is_lock_ness_monster(s):
    return any(x in s for x in ('tree fiddy', 'three fifty', '3.50'))


def is_loch_ness_monster2(string):
    return any(x for x in ["three fifty", "3.50", "tree fiddy"] if x in string )