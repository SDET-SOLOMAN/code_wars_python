...
def order_weight(word):
    print(word)
    print(sorted(word.split()))
    print(sorted(sorted(word.split(' ')), key=lambda x: sum(int(xx) for xx in x)))
    return ' '.join(sorted(sorted(word.split(' ')), key=lambda x: sum(int(xx) for xx in x)))